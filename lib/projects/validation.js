...
// Validates and normalizes list/query input. Invalid supplied values are
// rejected (400) rather than silently ignored. Absent fields are simply not
// included in `value` so the repository applies its defaults.
// Returns { valid, value, fields }.
export function validateListInput(body) {
  ...
  // categorySlug (optional; must be canonical when supplied).
  if (isPresent(input.categorySlug)) {
    if (!isValidCategorySlug(input.categorySlug)) {
      fields.categorySlug = "Categorie invalidă.";
    } else {
      value.categorySlug = String(input.categorySlug).trim();
    }
  }

  // statuses (optional array; every value must be a known status).
  if (input.statuses !== undefined) {
    if (!Array.isArray(input.statuses)) {
      fields.statuses = "statuses trebuie să fie o listă.";
    } else {
      const invalid = input.statuses.filter(
        (s) => !PROJECT_STATUSES.includes(s),
      );
      if (invalid.length > 0) {
        fields.statuses = "Unul sau mai multe statusuri sunt invalide.";
      } else {
        value.statuses = input.statuses.slice();
      }
    }
  }
  ...
  if (input.includeArchived !== undefined) {
    value.includeArchived = input.includeArchived === true;
  }

  // sort (optional; strict allowlist of sort keys).
  if (isPresent(input.sort)) {
    if (!Object.prototype.hasOwnProperty.call(PROJECT_SORT_COLUMNS, input.sort)) {
      fields.sort = "Cheie de sortare invalidă.";
    } else {
  ...
  if (isPresent(input.direction)) {
    if (input.direction !== "asc" && input.direction !== "desc") {
      fields.direction = "Direcție de sortare invalidă.";
    } else {
      value.direction = input.direction;
    }
  }

  // limit (optional; positive integer within max).
  if (isPresent(input.limit)) {
    const limit = Number(input.limit);
    if (!Number.isInteger(limit) || limit <= 0 || limit > PROJECT_MAX_LIMIT) {
      fields.limit = `limit trebuie să fie un întreg între 1 și ${PROJECT_MAX_LIMIT}.`;
    } else {
      value.limit = limit;
    }
  }

  // cursor (optional; non-negative integer offset, as string or number).
  if (isPresent(input.cursor)) {
  ...
  // search (optional; length-limited free text).
