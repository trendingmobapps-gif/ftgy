...
export function validateCreateInput(body) {
  ...
  for (const key of Object.keys(input)) {
    if (["safetyStatus", "isSafe", "moderated"].includes(key)) {
      fields[key] = "Câmpul nu este acceptat.";
    }
  }
  ...
  } else if (iconKey && !isAllowedProjectIconKey(iconKey)) {
    fields.iconKey = "iconKey nu este permis.";
  } else {
  ...
  } else if (accentKey && !isAllowedProjectAccentKey(accentKey)) {
    fields.accentKey = "accentKey nu este permis.";
  } else {
    value.accentKey = normalizeProjectAccentKey(accentKey) || null;
  }
