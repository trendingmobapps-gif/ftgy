...
import { getAllowedProjectAccentKeys, getAllowedProjectIconKeys } from "./icon-catalog.js";
...
export const PROJECT_CATEGORY_GUIDANCE = [
  ...
  { slug: "universal", label: "Universal", hint: "orice obiectiv personal sau profesional care nu se potrivește strict unei categorii existente" },
];
  ...
  return {
    ...
    schema: {
      ...
      properties: {
        ...
        iconKey: {
          anyOf: [
            { type: "string", enum: getAllowedProjectIconKeys() },
            { type: "null" },
          ],
        },
        accentKey: {
          anyOf: [
            { type: "string", enum: getAllowedProjectAccentKeys() },
            { type: "null" },
          ],
        },
...
11. iconKey și accentKey = alege DOAR din listele permise de mai jos, pe baza obiectivului și categoriei. Nu inventa chei noi. Dacă nu ești sigur, returnează null și sistemul va rezolva determinist.
12. iconKey trebuie să reflecte subiectul proiectului (ex: cafenea → coffee/storefront, bac → book/graduation, slăbit → target/scale, lansare AI → rocket/sparkles/brain).
13. accentKey trebuie să fie compatibil vizual cu iconKey și categoria.

Icon keys permise:
${getAllowedProjectIconKeys().join(", ")}

Accent keys permise:
${getAllowedProjectAccentKeys().join(", ")}
