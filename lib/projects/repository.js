// Supabase data-access layer for Projects. Uses the Supabase REST API via
// fetch with the service-role key (server-side only). Service-role bypasses
// RLS, so EVERY query here enforces `user_id = <owner>` explicitly. UUIDs are
// validated by callers before reaching this layer; values are always sent as
// encoded query parameters or JSON bodies (never string-interpolated SQL).
...
// --- Low-level REST helpers (never throw) --------------------------------
...
// Removes PostgREST filter-breaking characters from free-text search input.
...
// Inserts a new project owned by userId. Returns { ok, status, project, data }.
export async function createProject({
  ...
  const url = `${baseUrl}/rest/v1/${TABLE}?select=${encodeURIComponent(PROJECT_SELECT_COLUMNS)}`;
...
// Fetches a single project by id, scoped to userId. Returns
// { ok, status, project|null }.
export async function getProjectOwned({
  ...
  const query =
    ...
    `&select=${encodeURIComponent(PROJECT_SELECT_COLUMNS)}` +
    `&limit=1`;
...
// Lists projects owned by userId with filtering, search, sorting, pagination.
// Returns { ok, status, rows, count, nextCursor }.
export async function listProjects({ baseUrl, secretKey, userId, filters }) {
  ...
  // Category filter (validated by caller; still encoded here).
  if (f.categorySlug) {
    params.push(`category_slug=eq.${encodeURIComponent(f.categorySlug)}`);
  }

  // Case-insensitive search across name and goal.
  ...
  // Pagination (offset-based; cursor is a numeric offset string).
  ...
  params.push(`select=${encodeURIComponent(PROJECT_SELECT_COLUMNS)}`);
  params.push(`limit=${limit}`);
  params.push(`offset=${offset}`);

  const result = await supabaseFetch(
    `${baseUrl}/rest/v1/${TABLE}?${params.join("&")}`,
    {
      method: "GET",
  ...
  // Parse total count from the Content-Range header ("0-24/57").
...
// Updates allowlisted columns on a project scoped to userId. `columns` is a
// snake_case object. Returns { ok, status, project|null }.
