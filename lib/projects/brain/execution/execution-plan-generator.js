...
function normalizeMode(mode) {
  ...
  if (normalized === "checklist") return "structured_form";
...
export function normalizeExecutionPlan(raw, context) {
  ...
  const plan = {
    ...
    completionCriteria: {
      ...
      minimumResponses: Number.isFinite(raw.minimumResponses) ? raw.minimumResponses : null,
      requiresUserAcceptance: raw.requiresUserAcceptance !== false,
    },
...
export async function generateExecutionPlan({
  ...
  if (normalized.ok) {
    return { ok: true, plan: normalized.plan, source: "ai_generated" };
  }
