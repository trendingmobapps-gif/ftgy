...
import { defaultRequireGeneratedResultForMode } from "./completion-defaults.js";
import {
  repairPlanForAssistantValue,
  validateAssistantValueContribution,
} from "./assistant-value-validation.js";
...
import { PROJECT_MODEL_ROLES, resolveRuntimeRoleFromLegacyOperation } from "../project-model-policy.js";
import {
  buildExecutionPlanEvidenceInput,
  shouldReuseExecutionPlan,
  withExecutionPlanEvidence,
...
} from "./guided-question-contract.js";
...
function normalizeQuestion(question, index) {
  ...
  const normalized = {
    ...
    prompt,
    description: normalizeText(question?.description, 500) || null,
    recommendation: normalizeText(question?.recommendation, 500) || null,
    exampleAnswer: normalizeText(question?.exampleAnswer, 80) || null,
    required: question?.required !== false,
    weight: 1,
  };

  normalized.placeholder = normalizeGuidedQuestionPlaceholder({
    prompt,
    recommendation: normalized.recommendation,
    placeholder: question?.placeholder,
    exampleAnswer: normalized.exampleAnswer,
...
function buildCompletionCriteriaForMode(mode, raw = {}) {
  ...
  // The requireGeneratedResult default comes from the centralized table shared
  // with the completion evaluator. Explicit plan criteria always win.
  const requireGeneratedResult =
    raw.requireGeneratedResult ?? defaultRequireGeneratedResultForMode(mode);
  ...
  if (mode === "choice") {
    return {
      ...criteria,
      requireGeneratedResult,
      requireUserAcceptance: raw.requireUserAcceptance ?? true,
      requireChoice: true,
    };
  }
  if (mode === "recommendation_selection") {
    return {
      ...
      requireGeneratedResult,
      requireUserAcceptance: raw.requireUserAcceptance ?? true,
      requireUserReview: raw.requireUserReview ?? true,
      requireChoice: false,
    };
  }
  if (mode === "assessment") {
    return {
      ...criteria,
      requireGeneratedResult: defaultRequireGeneratedResultForMode(mode),
...
function resolveFinalizeActionLabel(mode) {
  ...
  if (mode === "recommendation_selection") return "Finalizează selecția";
...
function normalizeRecommendation(item, index) {
  ...
  const id = slugify(item?.id || title, index);
  return {
    id,
    title,
    explanation: normalizeText(item?.explanation, 600) || title,
...
function normalizeRecommendationGroup(group, index) {
  const title = normalizeText(group?.title, 200);
  if (!title) return null;
  const id = slugify(group?.id || title, index);
  const recommendations = (Array.isArray(group?.recommendations) ? group.recommendations : [])
    .map((item, itemIndex) => normalizeRecommendation(item, itemIndex))
    .filter(Boolean);
  if (recommendations.length < 1) return null;
  return {
    id,
    title,
    description: normalizeText(group?.description, 400) || null,
    recommendations,
  };
}

function normalizeSelectionRules(raw = {}) {
  return {
    minimumSelections: Number.isFinite(raw.minimumSelections) ? raw.minimumSelections : 1,
    maximumSelections: Number.isFinite(raw.maximumSelections) ? raw.maximumSelections : null,
    allowCustomOption: raw.allowCustomOption !== false,
    allowReorder: raw.allowReorder !== false,
  };
}
...
export function normalizeExecutionPlan(raw, context) {
  ...
  const recommendationGroups = (Array.isArray(raw.recommendationGroups) ? raw.recommendationGroups : [])
    .map((group, index) => normalizeRecommendationGroup(group, index))
    .filter(Boolean);

  const selectionRules = normalizeSelectionRules(raw.selectionRules || {});
...
function inferUserActionType(mode) {
  ...
  if (mode === "choice" || mode === "recommendation_selection") return "select";
...
function defaultPrimaryLabel(mode) {
  ...
  if (mode === "recommendation_selection") return "Confirmă strategia de canale";
...
function validatePlanShape(plan, context) {
  ...
  if (plan.mode === "recommendation_selection") {
    const count = (plan.recommendationGroups || []).reduce(
      (sum, group) => sum + (group.recommendations || []).length,
      0,
    );
    if (count < 2) return "recommendation_needs_items";
  }
  if (
...
export function buildContextualExecutionPlanFallback(context, preparation, memoryMap = new Map()) {
  ...
  const isStrategicRecommendation =
  ...
  if (isStrategicRecommendation) {
    const repaired = repairPlanForAssistantValue({ mode: "structured_form", requiredInputs: [] }, context);
    return finalizeFallbackPlan({
      ...repaired,
      source: "contextual_fallback",
    });
...
function finalizeFallbackPlan(partial) {
  const plan = {
    ...
    recommendationGroups: partial.recommendationGroups || [],
    selectionRules: partial.selectionRules || normalizeSelectionRules({}),
...
async function callOpenAiExecutionPlan({
  context,
  fetchImpl,
  apiKey,
  executionDecision,
  ...
  const classified = classifyOpenAiOperationComplexity({
    role: resolveRuntimeRoleFromLegacyOperation("executionPlan", "standard"),
    context,
    executionDecision,
    operationContext,
  });
  ...
  const role = resolveRuntimeRoleFromLegacyOperation("executionPlan", complexity);
  const baseUserPrompt = buildExecutionPlanUserPrompt(context);

  const structured = await callProjectStructuredJson({
    operation: "executionPlan",
    role,
    systemPrompt: buildExecutionPlanSystemPrompt(),
    userPrompt: baseUserPrompt,
    jsonSchema: buildExecutionPlanJsonSchema(),
    fetchImpl,
    apiKey,
    timeoutMs: EXECUTION_PLAN_TIMEOUT_MS,
    complexity,
    reasonCode,
    projectId,
    stepId,
    ...
    buildRepairUserPrompt: (reason) =>
      `${baseUserPrompt}\n\nRepară JSON-ul anterior. Motiv: ${reason || "invalid"}. Fă mai multă muncă de expert tu; oferă recomandări concrete, nu formulare goale.`,
    validateParsed: (parsed) => {
      const normalized = normalizeExecutionPlan(parsed, context);
      if (normalized.ok) {
        return { ok: true, value: normalized.plan };
      }
  ...
  return {
    ...
    source: structured.repairUsed ? "repair" : "openai",
  };
}
...
export async function generateExecutionPlan({
  ...
  const evidenceHash = buildExecutionPlanEvidenceInput({
    actionId,
    step,
    project,
    preparation,
  ...
  const first = await callOpenAiExecutionPlan({
    context,
    fetchImpl,
    apiKey,
    executionDecision,
  ...
  let normalized = first.ok ? { ok: true, plan: first.parsed } : { ok: false, reason: first.reason };
  let source = first.ok ? first.source || "openai" : first.reason;

  if (!normalized.ok) {
    return {
      ok: true,
      plan: buildContextualExecutionPlanFallback(context, preparation, memoryMap),
      source: "contextual_fallback",
    };
  }

  const valueEval = validateAssistantValueContribution(normalized.plan, context);
  if (!valueEval.valid) {
    const repairedRaw = repairPlanForAssistantValue(normalized.plan, context);
    const repairedNormalized = normalizeExecutionPlan(repairedRaw, context);
    ...
    requiredInputs = [
      {
        ...
        placeholder: question.placeholder || normalizeGuidedQuestionPlaceholder({
...
export function serializeInteractivePayloadFromPlan(plan) {
  ...
  const generatorModesWithInputs =
    (plan.mode === "generator" ||
      plan.mode === "document_builder" ||
      plan.mode === "image_generation" ||
      plan.mode === "upload_and_review") &&
    (plan.requiredInputs || []).length > 0;

  if (!isInteractivePlanMode(plan.mode) && !generatorModesWithInputs) return null;
  ...
  if (plan.mode === "guided_questions") {
    return {
      ...
      questions: (plan.questions || []).map((question) => serializeGuidedQuestionForClient(question)),
    };
  }
  ...
  if (plan.mode === "recommendation_selection") {
    return {
      ...
      groups: (plan.recommendationGroups || []).map((group) => ({
        id: group.id,
        title: group.title,
      ...
      selectionRules: plan.selectionRules || normalizeSelectionRules({}),
    };
  }
...
export async function ensureExecutionPlan({
  ...
  const evidenceHash = buildExecutionPlanEvidenceInput({
    ...
    executionDecision,
    workflowVersion:
      preparation?.context?.workflowVersion ||
      preparation?.context?.brainVersion ||
      project?.brain_version ||
      null,
    brainDecisionEvidenceHash:
      action.prepared_input?._brainDecisionEvidenceHash || null,
    memoryVersions: preparation?.context?.memoryVersions || null,
    resultReferences: (completedSteps || []).map((completedStep) => ({
      stepId: completedStep?.id || completedStep?.stepId || null,
      resultId: completedStep?.resultId || completedStep?.acceptedResultId || null,
      version: completedStep?.version || completedStep?.updatedAt || null,
    })),
    acceptedOutputReferences:
      preparation?.context?.acceptedOutputReferences || [],
  });
  let shouldGenerate = forceRegenerateInvalidPlan || !plan;

  if (plan && !shouldGenerate) {
    const reuseDecision = shouldReuseExecutionPlan({
    ...
    } else if (reuseDecision.reason === "execution_plan_evidence_changed") {
      shouldGenerate = true;
    }
  }
  ...
  if (shouldGenerate) {
    const priorHash = action.prepared_input?._executionPlanEvidenceHash || null;
    if (priorHash && priorHash === evidenceHash) {
      logOpenAiUsageEvent(() => {}, {
    ...
    const generated = await generateExecutionPlan({
      ...
      operationContext: {
        evidenceHash,
        authoritativeDecisionContract: false,
      },
    });
    ...
    const patch = {
      prepared_input: withExecutionPlanEvidence(withExecutionPlan(action.prepared_input, plan), {
        evidenceHash,
      }),
    };
    ...
    if (schemaCapabilities?.sessionColumns === false) {
      ...
      return { ok: true, action, plan, evidenceHash, persisted: false };
    }
    ...
    if (!updated.ok || !updated.action) {
      return {
        ...
        reason: "execution_plan_persistence_failed",
        action,
        plan: null,
      };
    }
  ...
  return { ok: true, action, plan, evidenceHash };
}
