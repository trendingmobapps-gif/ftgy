...
function buildCompletionCriteriaForMode(mode, raw = {}) {
  ...
  if (mode === "choice") {
    return {
      ...criteria,
      requireGeneratedResult: raw.requireGeneratedResult ?? true,
...
async function callOpenAiExecutionPlan({ context, fetchImpl, apiKey, repair = false, repairReason = null }) {
  const userPrompt = repair
    ? `${buildExecutionPlanUserPrompt(context)}\n\nRepară JSON-ul anterior. Motiv: ${repairReason || "invalid"}. Fă mai multă muncă de expert tu; oferă recomandări concrete, nu formulare goale.`
    : buildExecutionPlanUserPrompt(context);

  return callProjectStructuredJson({
