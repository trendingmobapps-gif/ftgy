...
function resolvePrimaryActionLabel(mode, session, requiredInputs) {
...
export function buildExecutionDefinition({
  ...
  return {
    ...
    primaryActionLabel: resolvePrimaryActionLabel(mode, session, requiredInputs),
    researchStatus,
    assessmentQuestionCount: mode === "assessment" ? Math.min(6, Math.max(3, requiredInputs.length || 4)) : null,
  };
}
