...
export function buildExecutionPrompt({ preparation, acceptedInput = {} }) {
  ...
  const context = preparation?.context || {
    project: { name: "Proiect", goal: "", categorySlug: null },
    step: { title: "Pas", expectedOutcome: "", description: "" },
    completedSteps: [],
  };
  const { preparedPrompt } = preparation;
...
`,
    userPrompt: `
Proiect: ${context.project?.name || "Proiect"}
...
Cerere pregătită:
${mergedInput.prompt || acceptedInput.prompt || preparedPrompt || ""}
`,
  };
}
