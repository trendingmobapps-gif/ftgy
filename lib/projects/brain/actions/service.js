...
export async function saveExecutionProgress({
  ...
  const updated = await updateActionSession({
    ...
    patch: {
      ...
      session_status: validation.canFinalize ? "ready_to_finalize" : actionRow.action.session_status || "collecting",
      status: "in_progress",
    },
  });
...
export async function reviewProjectSession({
  ...
  if (decision === "accept") {
    ...
    const updated = await updateActionSession({
      ...
      patch: {
        status: "in_progress",
        session_status: "ready_to_finalize",
        conversation,
        collected_input: collectedInput,
        pending_result_id: null,
      },
    ...
    const session = serializeSession({
      action: { ...acceptedAction, session_status: "ready_to_finalize" },
      preparation,
