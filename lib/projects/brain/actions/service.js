...
async function prepareDeterministicBrainDecision({
  ...
  if (!action || state.decisionReused) {
    return { action, enabled: true, state };
  }

  if (schemaCapabilities?.sessionColumns === false) {
    return {
      action: { ...action, prepared_input: preparedInput },
      enabled: true,
      state,
    };
  }
  ...
  if (!updated.ok || !updated.action) {
    logPrepareWarning(
      ...
      "Decision persistence failed; continuing with in-memory decision",
      {
  ...
  return { action: updated.action, enabled: true, state };
}
