...
import { ensureExecutionPlan, getExecutionPlanFromPreparedInput } from "../execution/execution-plan-generator.js";
...
import {
  ...
  validateExperienceProgressRequest,
...
} from "../execution/experience-idempotency.js";
...
} from "../workflow-adaptation-gate.js";
...
import {
  ...
  buildExecutionPlanEvidenceInput,
  readModelUsageState,
  shouldReusePersistedResult,
  withModelUsageState,
  withResultIdempotency,
  incrementFrontierUsageState,
} from "../openai-model-reuse.js";
import { computeResultGenerationEvidenceHash } from "../openai-evidence-hash.js";
import { logOpenAiUsageEvent } from "../openai-usage-observability.js";
import { isFrontierModel } from "../openai-model-tiers.js";
import { logPrepareFailure, logPrepareStage, logPrepareWarning, createPrepareStageUsageLogger } from "./prepare-stage-log.js";

const logPrepareStructuredEvent = createPrepareStageUsageLogger(logPrepareStage);
...
function stripPrivateBrainDecisionFields(preparedInput) {
  const safe = { ...(preparedInput || {}) };
  delete safe._brainDecision;
  delete safe._brainDecisionVersion;
  delete safe._brainDecisionEvidenceHash;
  return safe;
}

function serializePreparationForClient(preparation) {
  if (!preparation) return preparation;
  return {
    ...preparation,
    preparedInput: stripPrivateBrainDecisionFields(preparation.preparedInput),
  };
}
...
function attachAdaptiveResponse(base, adaptive, extras = {}) {
  ...
  const response = {
    ...base,
    preparation: serializePreparationForClient(base.preparation),
    executionDecision: adaptive.executionDecision,
...
async function prepareDeterministicBrainDecision({
  adaptive,
  action,
  project,
  step,
  ...
  if (!isProjectBrainDecisionLayerEnabled()) {
    return { ok: true, action, enabled: false };
  }
  ...
  const evidence = retrieveProjectBrainDecisionContext({
    ...
    workflow,
    memoryMap: adaptive.memoryMap,
    memoryVersions: adaptive.memoryVersions,
    reusableResource: adaptive.reusableResource,
    resultsByStepId,
    preparation: adaptive.adaptedPreparation,
    executionDecision: adaptive.executionDecision,
    // Step 1 records the absence of these future sources; it never invents data.
    knowledgeAvailable: false,
    researchAvailable: false,
    retrievalStatus: {
      resourcesChecked: schemaCapabilities?.adaptiveTables !== false,
    },
  });
  const state = buildOrReuseProjectBrainDecision({
    evidence,
    persistedPreparedInput: action?.prepared_input || {},
    actionId: action?.id || null,
  });
  const preparedInput = withProjectBrainDecision(
    action?.prepared_input || adaptive.adaptedPreparation?.preparedInput || {},
    state,
  );
  ...
  adaptive.brainDecision = state.decision;
  adaptive.brainDecisionMetadata = serializeSafeProjectBrainDecision(state.decision);
  adaptive.brainDecisionEvidenceHash = state.evidenceHash;
  adaptive.brainDecisionReused = state.decisionReused;

  if (!action || state.decisionReused) {
    return { ok: true, action, enabled: true, state };
  }

  if (schemaCapabilities?.sessionColumns === false) {
    return { ok: true, action, enabled: true, state, persisted: false };
  }
  ...
  const updated = await updateActionSession({
    ...
    userId,
    actionId: action.id,
    patch: { prepared_input: preparedInput },
  });

  if (!updated.ok || !updated.action) {
    logPrepareWarning(
      "brain_decision_persisted",
      "Decision persistence failed; action design will not be returned as successful",
      {
    ...
    return {
      ok: false,
      code: "ACTION_DESIGN_PERSISTENCE_FAILED",
      action,
      enabled: true,
      state,
    };
  ...
  logPrepareStage("brain_decision_persisted", {
    projectId: project?.id || null,
    stepId: step?.id || null,
    actionId: action?.id || null,
    decisionId: state.decision.decisionId,
    decisionType: state.decision.decisionType,
    decisionReused: state.decisionReused,
  });
  return { ok: true, action: updated.action, enabled: true, state };
}
...
export async function prepareProjectAction({
  ...
  try {
    ...
    const snapshotState = await reconstructBrainSnapshot({
      baseUrl,
      secretKey,
      userId,
      project,
      ...
      roadmapEvidenceHash: bundle.workflow?.brain_version || null,
    });
    ...
    const brainDecision = await prepareDeterministicBrainDecision({
      adaptive,
    ...
    if (!brainDecision.ok) {
      return {
        ok: false,
        code: brainDecision.code || "ACTION_DESIGN_PERSISTENCE_FAILED",
        recoverable: true,
      };
    }
    existing.action = brainDecision.action;
    ...
    if (isStepCompleted(step)) {
      ...
      return {
        ...
        session: artifacts.session,
        preparation: serializePreparationForClient(adaptive.adaptedPreparation),
    ...
    let preEnsuredPlan = null;
    let preEnsuredAction = existing.action;
    const persistedPlanBeforeEnsure = getPersistedExecutionPlan(existing.action);
    const actionDesignEvidenceHash = buildExecutionPlanEvidenceInput({
      actionId: existing.action?.id || null,
      step,
      project,
      preparation: adaptive.adaptedPreparation,
      memoryMap: adaptive.memoryMap,
      executionDecision: adaptive.executionDecision,
      workflowVersion: bundle.workflow?.brain_version || null,
      brainDecisionEvidenceHash: existing.action?.prepared_input?._brainDecisionEvidenceHash || null,
    });
    const actionDesignGate = shouldGenerateActionDesign({
      snapshot: snapshotState.snapshot,
      stepId,
      preparedInput: existing.action?.prepared_input || {},
      evidenceHash: actionDesignEvidenceHash,
      forceRegenerateInvalidPlan,
    });

    logBrainSnapshotEvent(logPrepareStructuredEvent, {
      projectId,
      stepId,
      actionId: existing.action?.id || null,
      artifactType: "action_design",
      evidenceHash: actionDesignEvidenceHash,
      reuseHit: actionDesignGate.reuseHit === true,
      reuseReason: actionDesignGate.reason,
      generationTriggered: actionDesignGate.generate === true,
    });

    if (existing.action) {
      if (actionDesignGate.generate || !getExecutionPlanFromPreparedInput(existing.action.prepared_input)) {
        preEnsuredPlan = await ensureExecutionPlan({
          baseUrl,
          secretKey,
          userId,
          actionRow: existing.action,
        ...
        preEnsuredPlan = {
          ok: true,
          action: existing.action,
          plan: getExecutionPlanFromPreparedInput(existing.action.prepared_input),
          evidenceHash: actionDesignEvidenceHash,
          reused: true,
        };
      }
      if (!preEnsuredPlan.ok) {
        logPrepareFailure(
          preEnsuredPlan.code || "ACTION_DESIGN_PERSISTENCE_FAILED",
          new Error(preEnsuredPlan.reason || "action_design_persistence_failed"),
          stageContext,
        );
        return {
          ok: false,
          code: preEnsuredPlan.code || "ACTION_DESIGN_PERSISTENCE_FAILED",
          recoverable: true,
        };
      }
      preEnsuredAction = preEnsuredPlan.action;

      if (snapshotState.snapshot && preEnsuredPlan.action?.prepared_input) {
        updateStepBlueprintAfterActionDesign({
          snapshot: snapshotState.snapshot,
          stepId,
          preparedInput: preEnsuredPlan.action.prepared_input,
          evidenceHash: preEnsuredPlan.evidenceHash || actionDesignEvidenceHash,
        });
        await persistBrainSnapshotToMemory({
          baseUrl,
          secretKey,
          userId,
          projectId,
          snapshot: snapshotState.snapshot,
          logFn: logPrepareStructuredEvent,
        });
      }
    }

    const resolvedPlan = preEnsuredPlan?.plan || null;
    const persistedPlan = persistedPlanBeforeEnsure || getPersistedExecutionPlan(existing.action);
    const planModeChanged = Boolean(
      persistedPlanBeforeEnsure?.mode &&
        resolvedPlan?.mode &&
        persistedPlanBeforeEnsure.mode !== resolvedPlan.mode,
    );
    const shouldResume = shouldResumeExistingAction({
      step,
      action: existing.action,
      executionPlan: resolvedPlan,
      forceRegenerateInvalidPlan,
    });
    const shouldReplaceIncompatible =
      planModeChanged ||
      shouldReplaceIncompatiblePreparedAction({
        step,
        action: existing.action,
    ...
    logPrepareStage("existing_action_loaded", {
      ...
      shouldResume,
      shouldReplaceIncompatible,
      persistedPlanMode: persistedPlan?.mode || null,
      resolvedPlanMode: resolvedPlan?.mode || null,
      planModeChanged,
    });

    if (shouldReplaceIncompatible && existing.action) {
      ...
      const saved = await replacePreparedAction({
        baseUrl,
        secretKey,
        userId,
        preparation: adaptive.adaptedPreparation,
        ...
        workflow: bundle.workflow,
        sessionState,
        existingAction: preEnsuredAction || existing.action,
        schemaCapabilities,
      });
    ...
    if (shouldResume && existing.action) {
      const resumeAction = preEnsuredAction || existing.action;
      const artifacts = await loadSessionArtifacts({
        baseUrl,
        secretKey,
        userId,
        stepId,
        actionRow: resumeAction,
        preparation: adaptive.adaptedPreparation,
        schemaCapabilities,
      });
      ...
      return enrichResponseWithInteractiveState({
        response: attachAdaptiveResponse(
          {
            ok: true,
            action: serializeActionRow(resumeAction, {
              latestResult: artifacts.acceptedResult,
              session: artifacts.session,
            }),
            session: artifacts.session,
...
export async function executeProjectAction({
  ...
  try {
    ...
    const experience = getExperienceFromPreparedInput(actionRow.action.prepared_input);
    const idempotencyKey = normalizedInput?.idempotencyKey || acceptedInput?.idempotencyKey || null;

    if (experience && (acceptedInput?.type === "experience" || normalizedInput?.type === "experience")) {
      if (idempotencyKey) {
        const cached = getCachedExperienceExecution(actionId, idempotencyKey);
        if (cached) {
          stageLog("experience_idempotency_hit", { actionId, idempotencyKey });
          return cached;
        }
      }

      if (Array.isArray(normalizedInput?._experienceValidationErrors)) {
        stageLog("accepted_input_validation_failed", {
          mode: "experience",
          errors: normalizedInput._experienceValidationErrors.map((entry) => entry.code),
        });
        return {
          ok: false,
          code: "VALIDATION",
          fields: { experience: normalizedInput._experienceValidationErrors },
        };
      }

      const strictValidation = validateExperienceValues(experience, normalizedInput.values || {}, {
        strict: true,
      });
      if (!strictValidation.valid) {
        stageLog("accepted_input_validation_failed", {
          mode: "experience",
          errors: strictValidation.errors.map((entry) => entry.code),
        });
        return {
          ok: false,
          code: "VALIDATION",
          fields: { experience: strictValidation.errors },
        };
      }
    }
    ...
    const requiredPlanInputs = (executionPlan?.requiredInputs || []).filter(
      (field) => field?.required !== false && field?.id,
    );
    if (requiredPlanInputs.length > 0) {
      const missingFields = requiredPlanInputs
        .filter((field) => !String(normalizedInput?.[field.id] || "").trim())
        .map((field) => field.id);

      if (missingFields.length > 0) {
        stageLog("accepted_input_validation_failed", {
          mode: executionPlan?.mode || executionMode || null,
          missingFields,
          requiredInputCount: requiredPlanInputs.length,
        });
        return {
          ok: false,
          code: "VALIDATION",
    ...
    const generationGate = resolveExecutionGenerationGate({
      action: actionRow.action,
      executionPlan,
    });

    if (!generationGate.allowed) {
      if (generationGate.code === "RESULT_ALREADY_PENDING" && generationGate.pendingResultId) {
        const pendingResult = await getActionResultById({
          baseUrl,
          secretKey,
          userId,
          resultId: generationGate.pendingResultId,
        });
        if (pendingResult.ok && pendingResult.result) {
          stageLog("pending_result_reused", { resultId: pendingResult.result.id });
          ...
          return {
            ...
            action: serializeActionRow(actionRow.action, {
              latestResult: pendingResult.result,
              session,
            }),
            result: serializeActionResultRow(pendingResult.result),
            session,
            requiresReview: true,
          };
        }
      ...
      if (generationGate.code === "EXECUTION_IN_PROGRESS") {
        stageLog("generation_in_progress_blocked", {
          elapsedMs: generationGate.elapsedMs,
          activeWindowMs: generationGate.activeWindowMs,
        });
        return {
          ok: false,
    ...
    const claim = await claimActionGenerationLock({
      baseUrl,
      secretKey,
      userId,
      actionId,
    ...
    if (!claim.claimed || !claim.action) {
      const refreshed = await getActionByStepId({ baseUrl, secretKey, userId, stepId });
      const refreshedGate = resolveExecutionGenerationGate({
        action: refreshed.action,
        executionPlan,
      });
      if (!refreshedGate.allowed && refreshedGate.code === "EXECUTION_IN_PROGRESS") {
        stageLog("generation_in_progress_blocked", {
          elapsedMs: refreshedGate.elapsedMs,
          activeWindowMs: refreshedGate.activeWindowMs,
          claimMiss: true,
        });
        return {
          ok: false,
      ...
      logExecuteFailure("session_loaded", new Error("generation_claim_failed"), {
        projectId,
        stepId,
        actionId,
        stage: currentStage,
        refreshedStatus: refreshed.action?.status || null,
        refreshedSessionStatus: refreshed.action?.session_status || null,
      });
      return { ok: false, code: "INTERNAL" };
    }

    actionRow.action = claim.action;
    ...
    if (adaptive.executionDecision.strategy === "reuse_resource" && adaptive.reusableResource) {
      ...
      const resultEvidenceHash = computeResultGenerationEvidenceHash({
        actionId,
        idempotencyKey,
        acceptedInput: normalizedInput,
        executionPlan,
      });
      const reuseDecision = shouldReusePersistedResult({
        preparedInput: actionRow.action.prepared_input,
        evidenceHash: resultEvidenceHash,
        idempotencyKey,
      });
      if (reuseDecision.reuse && reuseDecision.resultId) {
        const persistedResult = await getActionResultById({
          baseUrl,
          secretKey,
          userId,
          resultId: reuseDecision.resultId,
        });
        if (persistedResult.ok && persistedResult.result) {
          logOpenAiUsageEvent(createExecuteStageUsageLogger(stageLog), {
            role: "resultGeneration",
            reuseHit: true,
            reuseType: reuseDecision.reason,
            evidenceHash: resultEvidenceHash,
            projectId,
            actionId,
            success: true,
          ...
          generated = {
            ok: true,
            title: persistedResult.result.title,
            preview: persistedResult.result.preview,
            content: persistedResult.result.content,
            resultType: persistedResult.result.result_type || "text",
            strategy: "generate_resource",
            transport: "persisted_reuse",
            reused: true,
          };
        }
      }
      ...
      if (!generated) {
        const usageState = readModelUsageState(actionRow.action.prepared_input);
        generated = await generateActionResult({
          ...
          fetchImpl,
          logFn: stageLog,
          operationContext: buildResultGenerationOperationContext({
            executionPlan,
            resultIntent: executionPlan?.metadata?.resultIntent || null,
            outputLabel: executionPlan?.metadata?.outputLabel || executionPlan?.title || null,
            revisionContext: {
              revisionId: normalizedInput?.revisionId || null,
              parentResultId: normalizedInput?.parentResultId || null,
              authoritativeSourcePersisted: normalizedInput?.authoritativeSourcePersisted === true,
            },
            projectId,
            usageState,
          }),
    ...
    if (!generated.ok) {
      const isTimeout = generated.reason === "timeout";
      const recovery = isTimeout
        ? buildExecutionRecoveryMetadata({
            ...
            modelTier: generated.selectedModelTier || "frontier",
            reasoningEffort: generated.providerReasoningEffort || null,
            configuredTimeoutMs: generated.timeoutMs || null,
            elapsedMs: generated.elapsedMs || Date.now() - startedAt,
            abortSource: generated.abortSource || "operation_timeout_controller",
            providerCallCount: 1,
            resultExists: false,
      ...
      await updateActionSession({
        ...
        patch: isTimeout
          ? {
              ...
              prepared_input: withExecutionRecovery(actionRow.action.prepared_input || {}, recovery),
            }
          : {
              status: "failed",
      ...
      if (isTimeout) {
        const persistedState = resolvePersistedTimeoutRecoveryState({
          status: "prepared",
          session_status: "ready",
          prepared_input: withExecutionRecovery(actionRow.action.prepared_input || {}, recovery),
          collected_input: actionRow.action.collected_input || {},
          pending_result_id: null,
        });
        logGenerationTimeoutFailure(stageLog, {
          projectId,
          stepId,
          actionId,
          operation: "resultGeneration",
    ...
    if (!generated.reused) {
      const usageState = readModelUsageState(actionRow.action.prepared_input);
      const nextUsage = isFrontierModel(generated.model)
        ? incrementFrontierUsageState(usageState, { projectDelta: 1, actionDelta: 1 })
        : usageState;
      const ledger = buildResultIdempotencyLedger({
        actionId,
        idempotencyKey,
        acceptedInput: normalizedInput,
        resultId: savedResult.result.id,
        executionPlan,
      });
      await updateActionSession({
        baseUrl,
        ...
        actionId,
        patch: {
          prepared_input: withModelUsageState(
            withResultIdempotency(actionRow.action.prepared_input || {}, ledger),
            nextUsage,
          ),
        },
      });
    ...
    const executeCompletion = loadCompletionContext(reviewed.action, null, savedResult.result);
    ...
    const response = {
      ...
      session,
      requiresReview: true,
      canFinalize: executeCompletion.canFinalize,
      missingRequirements: executeCompletion.missingRequirements,
    };

    if (experience && idempotencyKey) {
      cacheExperienceExecution(actionId, idempotencyKey, response);
    }
...
async function applyProjectStepCompletion({
  ...
  const loadedAdaptation = await loadWorkflowAdaptationDecisionFromMemory({
    baseUrl,
    secretKey,
    userId,
    projectId,
  });
  const adaptationGate = evaluateWorkflowAdaptationGate({
    bundle,
    completedStep: step,
    acceptedResult,
    acceptedResource: registered.resource,
    memoryMap: memory.ok ? memory.map : new Map(),
    persistedAdaptation: loadedAdaptation.decision,
  });

  if (adaptationGate.persistDecision) {
    await persistWorkflowAdaptationDecisionToMemory({
      baseUrl,
      secretKey,
      userId,
      projectId,
      payload: {
        outcome: adaptationGate.outcome,
        evidenceHash: adaptationGate.evidenceHash,
        modelCallPermitted: adaptationGate.modelCallPermitted,
        requiresUserApproval: adaptationGate.requiresUserApproval === true,
        materialSignals: adaptationGate.materialSignals || [],
        recordedAt: new Date().toISOString(),
      },
    });
  }

  const partialScope = evaluatePartialRegenerationScope({
    changeSignals: adaptationGate.materialSignals || [],
    steps: bundle?.steps || [],
  });
  if (partialScope.mode === "localized_action_design_stale" && partialScope.affectedStepIds?.length) {
    const snapshotState = await reconstructBrainSnapshot({
      baseUrl,
      secretKey,
      userId,
    ...
    if (snapshotState.snapshot) {
      markAffectedStepDesignsStale(snapshotState.snapshot, partialScope.affectedStepIds);
      await persistBrainSnapshotToMemory({
        baseUrl,
        secretKey,
        userId,
  ...
  logBrainSnapshotEvent(() => {}, {
    ...
    stepId,
    workflowId: bundle?.workflow?.id || null,
    artifactType: "workflow_adaptation",
    evidenceHash: adaptationGate.evidenceHash,
    materialChangeDetected: adaptationGate.modelCallPermitted === true,
    generationTriggered: false,
    affectedStepCount: partialScope.affectedStepCount ?? 0,
  });

  let evolution = { changes: [] };
  if (adaptationGate.deterministicEvolutionPermitted) {
    evolution = evaluateWorkflowEvolution({
      bundle,
      completedStep: step,
      acceptedResource: registered.resource,
      memoryMap: memory.ok ? memory.map : new Map(),
...
export async function saveExecutionProgress({
  ...
  let collectedInput = { ...(actionRow.action.collected_input || {}) };

  if (progress.type === "experience" && experience) {
    const progressValidation = validateExperienceProgressRequest(progress, experience);
    if (!progressValidation.ok) {
      return { ok: false, code: "VALIDATION", fields: { progress: progressValidation.code } };
    }

    const existingValues = collectedInput.experience?.values || {};
    const mergedValues = mergeExperienceValues(existingValues, progress.values || {}, experience);
    const normalizedProgress = normalizeExperienceProgress(
      { ...progress, values: mergedValues },
      experience,
    );

    const legacyFormValues = experienceValuesToLegacyFormValues(experience, mergedValues);
  ...
  return {
    ...
    experienceProgress: collectedInput.experience || null,
  };
}
...
export async function reviewProjectSession({
  ...
  if (decision === "accept") {
    ...
    const collectedInput = {
      ...
      interactive: {
        ...
        // Persisted result evidence so a later prepare/refresh reconstructs
        // the same lifecycle deterministically.
        generatedResultId: resultId,
      },
    };
  ...
  if (decision === "reject" || decision === "cancel") {
    ...
    const rejectCompletion = loadCompletionContext(updated.action || actionRow.action, null, null);
    ...
    return {
      ...
      session,
      stepPending: true,
      canFinalize: rejectCompletion.canFinalize,
      missingRequirements: rejectCompletion.missingRequirements,
    };
  }
  ...
  if (decision === "improve") {
    ...
    const improveCompletion = loadCompletionContext(
      reviewed.action || actionRow.action,
      null,
      savedResult.result,
    );
    ...
    return {
      ...
      session,
      requiresReview: true,
      canFinalize: improveCompletion.canFinalize,
      missingRequirements: improveCompletion.missingRequirements,
    };
  }
