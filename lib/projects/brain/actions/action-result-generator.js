  ...
  const selectedIds = Array.isArray(selected)
    ? selected
        .map((item) => {
          ...
          if (item && typeof item === "object") return item.id || item.recommendationId || null;
          return null;
        })
        .filter(Boolean)
    : [];
...
export function buildRecommendationSelectionResultContent({ plan, interactive }) {
  ...
  const priorityOrderRaw = Array.isArray(interactive?.priorityOrder) ? interactive.priorityOrder : [];
  const priorityOrder = priorityOrderRaw
    .map((item) => {
      if (typeof item === "string") return item;
      if (item && typeof item === "object") return item.id || item.recommendationId || null;
      return null;
...
export async function generateActionResult({
  ...
  const generated = await executePreparedAction({
    ...
    operationContext: buildResultGenerationOperationContext({
      executionPlan,
      resultIntent: operationContext.resultIntent || executionPlan?.metadata?.resultIntent || null,
      outputLabel: operationContext.outputLabel || executionPlan?.metadata?.outputLabel || executionPlan?.title || null,
      revisionContext: operationContext.revisionContext || {},
      projectId: project?.id || operationContext.projectId || null,
      usageState: {
  ...
  const normalized = normalizeActionResultPayload({
    raw: {
      ...
      metadata: {
        ...
        model: generated.model || null,
        transport: generated.transport || null,
        providerStatus: generated.providerStatus || null,
        incompleteReason: generated.incompleteReason || null,
        requiresContinuation: Boolean(generated.requiresContinuation),
        continuedFromIncomplete: Boolean(generated.continuedFromIncomplete),
        incomplete: generated.providerStatus === "incomplete" || Boolean(generated.incompleteReason),
      },
    },
    step,
    project,
