...
import { ACTION_RESULT_SELECT_COLUMNS, ACTION_SELECT_COLUMNS } from "./constants.js";
import { appendActionHistory, buildActionArchiveSnapshot, extractCompatibleCollectedInput } from "./action-lifecycle.js";
...
export async function replacePreparedAction({
  baseUrl,
  secretKey,
  userId,
  preparation,
  ...
  nowIso,
  schemaCapabilities,
  replaceReason = "terminal_action_on_pending_step",
}) {
  const schema = await resolveSchemaCapabilities({ baseUrl, secretKey, schemaCapabilities });
  const now = nowIso || new Date().toISOString();
  const snapshot = buildActionArchiveSnapshot(existingAction, { reason: replaceReason });
  const preparedInput = appendActionHistory(preparation.preparedInput || {}, snapshot);
  const carriedCollectedInput = extractCompatibleCollectedInput(existingAction);
  ...
  if (schema.sessionColumns && sessionState) {
    ...
    row.conversation = sessionState.messages || [];
    row.collected_input = {
      ...(carriedCollectedInput || {}),
      ...(sessionState.collectedInput || {}),
    };
    row.pending_question = sessionState.pendingQuestion || null;
  }

  const patchQuery =
    `id=eq.${encodeURIComponent(existingAction.id)}` +
    `&user_id=eq.${encodeURIComponent(userId)}` +
    `&select=${encodeURIComponent(schema.actionSelectColumns)}`;
  ...
  if (!patch.ok) {
    logPrepareWarning("action_replaced", "Action replacement patch failed", {
      stepId: step.id,
      status: patch.status,
      supabaseError: extractSupabaseErrorPayload(patch.data),
    });
...
export async function updateActionSession({
  ...
  return { ok: Boolean(action), action, data: result.data, status: result.status };
}

/**
 * Atomically claim generation from prepared → in_progress.
 * Uses Supabase row filter so only one concurrent instance can claim.
 */
export async function claimActionGenerationLock({
  baseUrl,
  secretKey,
  userId,
  actionId,
...
export async function insertActionResult({
  ...
  return { ok: Boolean(saved), result: saved, data: result.data, status: result.status };
}

export async function getActionResultById({ baseUrl, secretKey, userId, resultId }) {
  if (!resultId) {
...
export function serializeActionRow(row, { latestResult = null, session = null } = {}) {
  if (!row) return null;
  const {
    _brainDecision: _privateBrainDecision,
    _brainDecisionVersion: _privateBrainDecisionVersion,
    _brainDecisionEvidenceHash: _privateBrainDecisionEvidenceHash,
    ...clientPreparedInput
  } = row.prepared_input || {};
  ...
  return {
    ...
    preparedInput: clientPreparedInput,
