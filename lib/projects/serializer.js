...
export function serializeProject(row) {
  ...
  return {
    ...
    brainStatus: emptyToNull(row.brain_status) || "pending",
    brainVersion: emptyToNull(row.brain_version),
    brainGeneratedAt: emptyToNull(row.brain_generated_at),
    brainFailureCode: emptyToNull(row.brain_failure_code),
    brainAttemptCount:
      typeof row.brain_attempt_count === "number" ? row.brain_attempt_count : 0,
  };
}
