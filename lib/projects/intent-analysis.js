...
export async function analyzeProjectIntent(input, deps = {}) {
  ...
  const deterministic = applyDeterministicIntentRules(input);
  if (deterministic) {
  ...
  if (normalized.ok) {
    ...
    if (guarded.ok) {
      if (
        ...
        isObviouslyClearGoal(input.goal)
      ) {
        const overrideReady = buildDeterministicReadyResult(input.goal, input);
        if (overrideReady) {
