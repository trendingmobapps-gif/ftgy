...
async function callOpenAiIntent({ goal, optionalName, clarificationAnswers, fetchFn = fetch, apiKey }) {
  ...
  try {
    const response = await fetchFn("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: buildIntentSystemPrompt({ categoryTools }) },
          {
            role: "user",
...
export async function analyzeProjectIntent(input, deps = {}) {
  ...
  // One repair attempt when model output is structurally invalid.
