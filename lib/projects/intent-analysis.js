...
import {
  applyDeterministicIntentRules,
  buildDeterministicReadyResult,
  isObviouslyClearGoal,
  validateClarificationQuestionsForGoal,
} from "./intent-deterministic-rules.js";
import {
...
function mapSafetyResultToIntentResult(safetyResult) {
  if (safetyResult.status === "blocked") {
    return toBlockedApiPayload(safetyResult);
  }

  if (safetyResult.status === "needs_safety_clarification") {
    return safetyResult.payload;
  }

  return null;
}

function stripSafetyClarificationAnswers(input) {
  const { category } = splitClarificationAnswers(input.clarificationAnswers);
  return {
    ...input,
    clarificationAnswers: category.length > 0 ? category : undefined,
  };
}
...
function normalizeReadyPayload(modelResult, input) {
  const rawCategory = typeof modelResult.categorySlug === "string" ? modelResult.categorySlug.trim() : "";
  const categorySlug = isValidIntentCategorySlug(rawCategory)
    ? rawCategory
    : "universal";
  ...
  if (typeof modelResult.iconKey === "string" && modelResult.iconKey.trim()) {
    payload.iconKey = modelResult.iconKey.trim();
  }
  if (typeof modelResult.accentKey === "string" && modelResult.accentKey.trim()) {
    payload.accentKey = modelResult.accentKey.trim();
  }

  return { ok: true, payload: attachResolvedIconsToReadyPayload(payload, input) };
}

function normalizeClarificationPayload(modelResult, input) {
  const message =
  ...
  const questions = sanitizeIntentQuestions(modelResult.questions);
  const validation = validateClarificationQuestionsForGoal(input.goal, questions);

  if (!validation.ok) {
    const deterministicReady = buildDeterministicReadyResult(input.goal, input);
    if (deterministicReady) {
      return deterministicReady;
    }

    return {
      status: "unsupported",
      message: "Nu am putut pregăti o clarificare relevantă pentru acest obiectiv. Reformulează obiectivul mai concret.",
    };
  }
...
export function normalizeIntentModelResult(modelResult, input) {
  ...
  if (status === "needs_clarification") {
    return { ok: true, payload: normalizeClarificationPayload(modelResult, input) };
  }
...
export async function analyzeProjectIntent(input, deps = {}) {
  ...
  const useSafetyAi = !deps.fetchFn || Boolean(deps.safetyFetchFn) || Boolean(deps.enableSafetyAi);
  const safetyDecision = await evaluateProjectSafety(input, {
    apiKey: useSafetyAi ? deps.apiKey || process.env.OPENAI_API_KEY || "" : "",
    fetchFn: deps.safetyFetchFn,
  ...
  const safetyMapped = mapSafetyResultToIntentResult(safetyDecision);
  if (safetyMapped) {
    return { ok: true, result: safetyMapped };
  }

  const intentInput = stripSafetyClarificationAnswers(input);
  const deterministic = applyDeterministicIntentRules(intentInput);
  if (deterministic) {
  ...
  const first = await callOpenAiIntent({
    goal: intentInput.goal,
    optionalName: intentInput.optionalName,
    clarificationAnswers: intentInput.clarificationAnswers,
    fetchFn,
    apiKey,
  });
  ...
  let normalized = normalizeIntentModelResult(first.parsed, intentInput);

  if (normalized.ok) {
    const guarded = applyClarificationRoundGuard(normalized, intentInput);
    if (guarded.ok) {
      if (
        guarded.payload.status === "needs_clarification" &&
        isObviouslyClearGoal(intentInput.goal)
      ) {
        const overrideReady = buildDeterministicReadyResult(intentInput.goal, intentInput);
        if (overrideReady) {
          return { ok: true, result: overrideReady };
        }
      }

      return { ok: true, result: guarded.payload };
    }
  } else if (!hasClarificationAnswers(intentInput)) {
    ...
    normalized = normalizeIntentModelResult(repair.parsed, intentInput);
    if (normalized.ok) {
      const guarded = applyClarificationRoundGuard(normalized, intentInput);
      if (guarded.ok) {
        return { ok: true, result: guarded.payload };
  ...
  if (hasClarificationAnswers(intentInput)) {
    ...
    const finalNormalized = normalizeIntentModelResult(finalRepair.parsed, intentInput);
